...
xpdb_to_simpar <- function(xpdb){
  ...
  list(
    ...
    omega = omega,
    sigma = sigma,
    odf = mapply(compute_df, omega, matrix_to_list(p_cor$omega, blockform = blocks$omega)),
    sdf = mapply(compute_df, sigma, matrix_to_list(p_cor$sigma, blockform = blocks$sigma))
  )
}
